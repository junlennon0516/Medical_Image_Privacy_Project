...
int main() {
	...
	// 서버와 동일한 파라미터 사용
	// Poly Modulus Degree: 8192 (더 안정적인 파라미터)
	// Coeff Modulus: SEAL 표준 값 사용
	// Scale: 2^30 (성능 최적화)
	size_t poly_modulus_degree = 8192;
	parms.set_poly_modulus_degree(poly_modulus_degree);
	// 서버와 동일: {50, 40, 40, 50} (SEAL 표준 값)
	...
	// 1024개의 픽셀 값이 필요함 (32x32 이미지)
	if (input_data.size() != 1024) {
		cout << "[Error] 데이터 개수가 올바르지 않습니다. (필요: 1024개 (32x32), 실제: " << input_data.size() << "개)\n";
		cout << "[Error] result_app.py에서 올바른 이미지를 업로드해주세요.\n";
		return 1;
	}

	cout << "[Client] Loaded " << input_data.size() << " normalized pixel values from result_app.py (64x64 image)\n";
